...
import { VOTINGSDAPP_PROGRAM_ADDRESS } from '../programs';
...
export type DecrementInstruction<
  TProgram extends string = typeof VOTINGSDAPP_PROGRAM_ADDRESS,
  TAccountVotingsdapp extends string | AccountMeta<string> = string,
  TRemainingAccounts extends readonly AccountMeta<string>[] = [],
> = Instruction<TProgram> &
  InstructionWithData<ReadonlyUint8Array> &
  InstructionWithAccounts<
    [
      TAccountVotingsdapp extends string
        ? WritableAccount<TAccountVotingsdapp>
        : TAccountVotingsdapp,
      ...TRemainingAccounts,
    ]
  >;
...
export type DecrementInput<TAccountVotingsdapp extends string = string> = {
  votingsdapp: Address<TAccountVotingsdapp>;
};

export function getDecrementInstruction<
  TAccountVotingsdapp extends string,
  TProgramAddress extends Address = typeof VOTINGSDAPP_PROGRAM_ADDRESS,
>(
  input: DecrementInput<TAccountVotingsdapp>,
  config?: { programAddress?: TProgramAddress }
): DecrementInstruction<TProgramAddress, TAccountVotingsdapp> {
  // Program address.
  const programAddress = config?.programAddress ?? VOTINGSDAPP_PROGRAM_ADDRESS;

  // Original accounts.
  const originalAccounts = {
    votingsdapp: { value: input.votingsdapp ?? null, isWritable: true },
  };
  ...
  const instruction = {
    accounts: [getAccountMeta(accounts.votingsdapp)],
    programAddress,
    data: getDecrementInstructionDataEncoder().encode({}),
  } as DecrementInstruction<TProgramAddress, TAccountVotingsdapp>;
